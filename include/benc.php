...
function bdec($s)
{
    if (preg_match('/^(\d+):/', $s, $m)) {
        ...
        $ss = mb_substr($s, 0, $pl + $l);

        if (mb_strlen($v) != $l) {
            return;
        }

        return ['type' => 'string', 'value' => $v, 'strlen' => mb_strlen($ss), 'string' => $ss];
    }
    ...
    if (preg_match('/^i(\d+)e/', $s, $m)) {
        ...
        if ('0' == $v[0] && 1 != mb_strlen($v)) {
            return;
        }

        return ['type' => 'integer', 'value' => $v, 'strlen' => mb_strlen($ss), 'string' => $ss];
    }
...
function bdec_list($s)
{
    ...
    return ['type' => 'list', 'value' => $v, 'strlen' => mb_strlen($ss), 'string' => $ss];
}
...
function bdec_dict($s)
{
    ...
    return ['type' => 'dictionary', 'value' => $v, 'strlen' => mb_strlen($ss), 'string' => $ss];
}
