...
function get_user_timezone($id)
{
    ...
    $query = $GLOBALS['xoopsDB']->queryF($sql);

    if ('0' != $GLOBALS['xoopsDB']->getRowsNum($query)) {
        $kasutaja = $GLOBALS['xoopsDB']->fetchBoth($query);
...
function get_row_count($table, $suffix = '')
{
    ...
    ($r = $GLOBALS['xoopsDB']->queryF("SELECT COUNT(*) FROM $table$suffix")) || die($GLOBALS['xoopsDB']->error());

    ($a = $GLOBALS['xoopsDB']->fetchRow($r)) || die($GLOBALS['xoopsDB']->error());
...
{
    $addLen = mb_strlen($needle);

    $endPos = $offset - $addLen;

    while (true) {
        if (false === ($newPos = mb_strpos($haystack, $needle, $endPos + $addLen))) {
            break;
        }
...
function format_quotes($s)
{
    while ($old_s != $s) {
        ...
        $open = _strlastpos(mb_substr($s, 0, $close), '[quote');

        if (false === $open) {
            return $s;
        }

        $quote = mb_substr($s, $open, $close - $open + 8);
        ...
        $s = mb_substr($s, 0, $open) . $quote . mb_substr($s, $close + 8);
    }
...
function format_comment($text, $strip_html = true)
{
    ...
    $s = preg_replace(
        "/\[nfo\]((\s|.)+?)\[\/nfo\]/i",
        "<tt><nobr><font face='MS Linedraw' size=2 style='font-size: 10pt; line-height: " . "10pt'>\\1</font></nobr></tt>",
        $s
    );
    ...
    while (list($code, $url) = each($smilies)) {
        $s = str_replace($code, "<img border=0 src=\"/pic/smilies/$url\" alt=\"" . htmlspecialchars($code, ENT_QUOTES | ENT_HTML5) . '">', $s);
    }
...
{
    return mktime(mb_substr($s, 11, 2), mb_substr($s, 14, 2), mb_substr($s, 17, 2), mb_substr($s, 5, 2), mb_substr($s, 8, 2), mb_substr($s, 0, 4));
}
...
function write_log($text)
{
    ...
    $GLOBALS['xoopsDB']->queryF("INSERT INTO sitelog (added, txt) VALUES($added, $text)") or sqlerr(__FILE__, __LINE__);
}
